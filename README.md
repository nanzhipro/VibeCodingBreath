...
下面这段提示词可在 Claude Code（或同等 agent）里一把生成与本仓库等价的 App：
...
claude \
  ...
  --verbose \
  -p "Please implement a macOS menu-bar mindful-breathing companion app called **VibeCodingBreath** (Swift 5.10, macOS 14+, Apple Silicon + Intel universal) with the following requirements:
